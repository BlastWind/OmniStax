...
import {
  CATEGORIES, CATEGORY_CHIP, atCursor, categoriesFor, faceOf, into, matches, move, open, out, rowsIn, typed, type PickerRow,
...
test('what is typed narrows the categories, and going in narrows their rows', () => {
  ...
  const inside = into(typed(open(), 'sec'), 'sections');
  assert.equal(inside.category, 'sections');
  assert.equal(inside.query, '', 'the words that named the category are not the words inside it');
  const face = faceOf(typed(inside, '16.4'), rows);
  assert.equal(face.kind, 'rows');
  assert.equal(face.rows.length, 1);
  assert.equal(out(typed(inside, '16.4')).category, null);
});

test('a query that names no category searches every row instead', () => {
  const face = faceOf(typed(open(), 'hooke'), rows);
  assert.equal(face.kind, 'rows');
