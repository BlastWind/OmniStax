/* A figure of the book dragged into a note. The panels drag their rows out with
   ...
   `![[fig:college-physics-2e/7.2:sim-area]]`, the book, the section and the id the section's own HTML gives
...
import type { SectionId, SectionRef } from '../../types/ids';
...
export const dragFigures = (root: HTMLElement, ref: SectionRef): void => {
  root.querySelectorAll<HTMLElement>(FIGURES).forEach((fig) => {
    ...
    dragout(head, { kind: 'figure', book: ref.book, section: ref.section, id: localFigureId(fig.id, ref.section) });
  });
};
