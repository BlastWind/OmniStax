...
import { focus, assumedBook } from '../sections/focus.svelte';
...
import { type SectionId, type SectionRef, type SpanId, sectionId, sectionRef, spanId, spanRef, conceptId, sheetId, sectionOfSpan, newViewItem, sheetItem, itemKey } from '../types/ids';
...
const sectionOf = (el: Element): SectionId => sectionId(el.closest<HTMLElement>('[data-sec]')?.dataset.sec ?? focus.section.section);
const inBook = (sec: SectionId): SectionRef => sectionRef(assumedBook(), sec);
const chapterData = (sec: SectionId) => { const dir = registry.chapterOf(inBook(sec))?.dir; return dir ? registry.chapter(assumedBook(), dir) : undefined; };
const goSpanId = (id: SpanId): void => goSpan(spanRef(assumedBook(), id));
...
const spanTitle = (id: SpanId): string | undefined => headingText(findEl(assumedBook(), id)?.querySelector('h2, h3'));
...
export const spanName = (id: SpanId): string | undefined => {
  const h = findEl(assumedBook(), id)?.querySelector('h2, h3'); if (!h) return undefined;
...
const places = (ids: readonly SpanId[]): { id: SpanId; title: string }[] => {
  ...
  return [...named, ...secs.map((sec) => ({ id: rest.find((id) => sectionOfSpan(id) === sec)!, title: `${sec} · ${registry.entry(inBook(sec))?.title ?? 'section'}` }))];
};
...
const showOriginal = (figure: SpanId): void => {
  goSpanId(figure);
  const show = () => { const f = findEl(assumedBook(), figure); const b = f?.querySelector<HTMLButtonElement>('button.fig-original'); if (f && b && !f.classList.contains('show-original')) b.click(); };
  const f = findEl(assumedBook(), figure); if (f) { show(); return; }
  openDoc(inBook(sectionOfSpan(figure)), 'text').then(() => requestAnimationFrame(show));
};
const openExternal = (sec: SectionId): void => { window.open(registry.entry(inBook(sec))?.openstax ?? registry.manifest(assumedBook()).openstax, '_blank', 'noopener'); };
...
const showElement = (symbol: string): void => {
  ...
  sheets.pin(symbol);
  void openItem(itemKey(sheetItem(assumedBook(), sheetId(entry.id))));
};
export const nav: Nav = {
  goSpan: goSpanId, openSection: (sec) => { openDoc(inBook(sec), 'text'); }, showView, showOriginal, openExternal, showElement,
};
...
const variable = (t: HTMLElement): Card | null => {
  ...
  const type = typeOf(t); const typeLabel = type ? registry.manifest(assumedBook()).types[type]?.label : undefined;
  return variableCard({ sym, tex: registry.manifest(assumedBook()).symbols[sym] ?? sym, typeLabel, section: sec, formulasLoaded: !!data, variable: data ? lookupVariable(data.formulas.variables, sym, sec) : undefined }, nav);
};
const figure = (t: HTMLElement): Card | null => {
  const n = t.dataset.figref; const id = t.getAttribute('href')?.slice(1); if (!n || !id) return null;
  const fig = findEl(assumedBook(), id);
...
const term = (t: HTMLElement): Card | null => {
  ...
  const top = registry.state(inBook(home))?.docs.text?.querySelector<HTMLElement>('section[id]')?.id;   /* the section's first span, when it is loaded */
...
const reference = (t: HTMLElement): Card | null => {
  ...
  const el = findEl(assumedBook(), id); if (!el || !(el.matches('.example[id]') || el.matches('section[id]'))) return null;
...
const concept = (t: HTMLElement): Card | null => {
  ...
  const own = registry.concept(assumedBook(), id); const c = own ?? books.concept(id); if (!c) return null;
  ...
  return conceptCard({ concept: c, intro: places(sp.intro), uses: places(sp.uses), built: !!registry.entry(inBook(sec))?.built, onMap: t.matches('.node') }, nav);
};
