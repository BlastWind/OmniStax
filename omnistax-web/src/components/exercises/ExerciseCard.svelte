<script lang="ts">
  ...
  let {
    ...
    outcome = null, onanswer, session,
  }: {
    section: SectionId; ex: ExerciseDTO; hidden?: boolean; book?: string;
    outcome?: boolean | null; onanswer?: (ok: boolean) => void; session?: SessionId;
  } = $props();
  ...
  const record = (ok: boolean, self: boolean): void => {
    if (completed) return;
    practice.record(book, section, ex, ok, self, Date.now(), session);
    earned = ok ? 'Recorded as correct.' : 'Recorded as incorrect.';
    localOutcome = ok;
    localDone = true;
    onanswer?.(ok);
  };
