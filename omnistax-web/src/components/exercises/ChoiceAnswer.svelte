<script lang="ts">
  ...
  let { answer, name, oncheck, locked = null }: { answer: Extract<AnswerDTO, { type: 'choice' }>; name: string; oncheck?: (v: Verdict) => void; locked?: boolean | null } = $props();
...
<div class="actions"><button type="button" class="btn" disabled={done || picked === null} onclick={check}>Check</button></div>
