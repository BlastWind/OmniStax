<script lang="ts">
  /* The motion behind the clock. Before a session starts, sixty short segments
     drift on a slow noise field, each pointing the way it is going — the page
     is unsettled, and so are they. On start they relax into a ring around the
     clock, turn tangent to it, and the whole ring rotates once a minute, which
     is the second hand the digital face has not got.

     It is decoration, so it gives way: nothing is drawn while the tab is hidden,
     while the reader has asked for less motion, or while the app's animations
     are off. */
