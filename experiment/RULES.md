...
from `book.json` (title, authors, publisher, licence, the section's page at
the publisher) and from the section's `notes`. The pipeline never writes
attribution; it writes only `notes`: one plain sentence saying what the
section left out, or nothing. The long form (what Omnia changes, the
licence of the adapted pages, every section beside its original) is the
`/<book>/attribution/` page, also generated.
