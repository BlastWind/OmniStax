...
(function () {
  ...
  const T = ctl(d.controls, { label: '\\kT', cls: 't', min: 0.25, max: 4, step: 0.05, value: 0.5, unit: 's', dec: 2, onInput: reset, aria: 'period' });
  const W = ctl(d.controls, { label: '\\text{window}', cls: 't', min: 1, max: 5, step: 0.5, value: 2, unit: 's', dec: 1, onInput: reset, aria: 'counting window' });
  ...
  function draw() {
    ...
    if (Math.abs(x) > 0.15) { vbracket(ctx, cx + 40, ys, ys - u * AMP, C('x')); text(ctx, 'x = ' + (x > 0 ? '+' : '−') + fmt(Math.abs(x), 1) + ' mm', cx + 58, ys - AMP - 24, C('x'), { weight: 600, size: 20 }); }
    ...
    const { X, Y } = axes(ctx, box, [-S, 0], [-3, 3], { xl: 'time before now (s)', xc: C('t'), yl: 'x (mm)', yc: C('x'), nx: span.n, ny: 2, fx: (v) => (Math.abs(v) < 1e-9 ? 'now' : fmt(-v, dec)), fy: (v) => fmt(v, 0) });
    ctx.save(); ctx.fillStyle = alpha(C('t'), 0.12); ctx.fillRect(X(-W.v), box.t, X(0) - X(-W.v), box.b - box.t); ctx.restore();
    const t0 = Math.max(0, tau - S);
    if (tau > 0) curve(ctx, (s) => xAt(tau + s), t0 - tau, 0, X, Y, C('x'), 4, Math.min(3000, Math.ceil(40 * (tau - t0) / T.v) + 20));
    for (let n = Math.ceil(t0 / T.v - 1e-9); n * T.v <= tau + 1e-9; n++) dot(ctx, X(n * T.v - tau), Y(A), C('t'), true, 6);
    const n1 = Math.floor(tau / T.v + 1e-9);
    if (n1 >= 1 && (n1 - 1) * T.v >= t0 - 1e-9) hbracket(ctx, X((n1 - 1) * T.v - tau), X(n1 * T.v - tau), Y(A) - 26, C('t'), 'T = ' + fmt(T.v, 2) + ' s');
    text(ctx, 'the last ' + fmt(W.v, 1) + ' s hold ' + fmt(W.v / T.v, 2) + ' cycles', X(0) - 12, box.b - 16, C('t'), { weight: 600, size: 20, align: 'right' });
...
(function () {
  ...
  const tt = ctl(d.controls, { label: '\\kt', cls: 't', min: 2, max: 30, step: 0.5, value: 6, unit: 's', dec: 1, onInput: reset, aria: 'elapsed time' });
  const T = () => tt.v / N.v;
  ...
  function draw() {
    ...
    text(ctx, 't = ' + fmt(tau, 1) + ' s', sx, sy + r + 34, C('t'), { weight: 600, size: 26, align: 'center' });
    /* the counter */
    const kx = 1150;
