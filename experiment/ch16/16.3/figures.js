...
(function () {
  ...
  const X = ctl(d.controls, { label: '\\kX', cls: 'x', min: 0.02, max: 0.2, step: 0.01, value: 0.1, unit: 'm', dec: 2, onInput: reset, aria: 'amplitude' });
  ...
  function draw() {
    ...
    for (const [val, lab] of [[-X.v, 'x = −X'], [0, 'x = 0'], [X.v, 'x = +X']]) {
      const px = eq + val * SC; line(ctx, px, floorY + 24, px, floorY + 44, C('x'), 3);
      text(ctx, lab, px, floorY + 68, C('x'), { size: 20, weight: 600, align: 'center' });
    }
    ...
    if (Math.abs(Fn) > 0.02 * k.v * X.v) {
      ...
      arrow(ctx, bx, by - 66, bx + s * al, by - 66, C('F'), 5);
      text(ctx, 'F = −kx = ' + sgn(Fn) + fmt(Math.abs(Fn), 1) + ' N', bx + s * (al + 14), by - 66, C('F'), { weight: 600, size: 20, align: s < 0 ? 'right' : 'left' });
    }
    if (Math.abs(v) > 0.02 * vmax) {
      const al = 60 + 200 * Math.abs(v) / vmax, s = v < 0 ? -1 : 1;
      arrow(ctx, bx, by - 112, bx + s * al, by - 112, C('v'), 5);
      text(ctx, 'v = ' + sgn(v) + fmt(Math.abs(v), 2) + ' m/s', bx + s * (al + 14), by - 112, C('v'), { weight: 600, size: 20, align: s < 0 ? 'right' : 'left' });
    }
...
(function () {
  ...
  const k = ctl(d.controls, { label: '\\kk', cls: 'F', min: 10000, max: 200000, step: 1000, value: 65300, unit: 'N/m', dec: 0, onInput: reset });
  const X = ctl(d.controls, { label: '\\kX', cls: 'x', min: 0.02, max: 0.1, step: 0.005, value: 0.05, unit: 'm', dec: 3, onInput: reset, aria: 'amplitude' });
  const cy = cycle(() => Infinity, 0);
  function reset() { cy.reset(); }
  ...
  function draw() {
    ...
    for (const [i, amp, lab] of [[0, X.v, 'X = '], [1, X.v / 2, 'X/2 = ']]) {
      ...
      text(ctx, 'amplitude ' + lab + fmt(amp, 3) + ' m', 1290, floorY - 40, C('x'), { size: 20, weight: 600, align: 'right' });
    }
    line(ctx, eq, 70, eq, 370, PAL.muted, 2, [8, 8]); text(ctx, 'x = 0', eq, 388, C('x'), { size: 18, weight: 600, align: 'center' });
    /* the graph: T against m for this k */
    const Tr = nice(0, Tof(2000) * 1.05, 4), box = { l: 200, r: 1240, t: 440, b: 640 };
    const { X: gx, Y: gy } = axes(ctx, box, [0, 2000], [0, Tr.hi], { xl: 'mass m (kg)', xc: PAL.ink, yl: 'T (s)', yc: C('t'), nx: 4, ny: Tr.n, fx: (v) => fmt(v, 0), fy: (v) => fmt(v, 1) });
    curve(ctx, Tof, 0, 2000, gx, gy, C('t'), 4, 100);
    text(ctx, 'T = 2π√(m/k)', gx(1500) + 10, gy(Tof(1500)) + 38, C('t'), { weight: 600, size: 20 });
    line(ctx, gx(m.v), box.b, gx(m.v), gy(T()), PAL.ink, 2, [4, 8]); line(ctx, box.l, gy(T()), gx(m.v), gy(T()), C('t'), 2, [4, 8]);
    dot(ctx, gx(m.v), gy(T()), C('t'), true, 9);
...
(function () {
  ...
  const X = ctl(d.controls, { label: '\\kX', cls: 'x', min: 0.02, max: 0.1, step: 0.005, value: 0.05, unit: 'm', dec: 3, onInput: reset, aria: 'amplitude' });
  const T = ctl(d.controls, { label: '\\kT', cls: 't', min: 0.5, max: 3, step: 0.1, value: 1, unit: 's', dec: 2, onInput: reset, aria: 'period' });
  const cy = cycle(() => Infinity, 0);
  function reset() { cy.reset(); }
  ...
  function draw() {
    ...
    for (const [val, lab] of [[X.v, 'x = +X'], [0, '0'], [-X.v, '−X']]) { line(ctx, pr, yOf(val), pr + 12, yOf(val), C('x'), 3); text(ctx, lab, pr + 20, yOf(val), C('x'), { size: 20, weight: 600 }); }
    const t0 = Math.max(0, tau - S);
    if (tau > 0) curve(ctx, (s) => xAt(tau + s), t0 - tau, 0, Xp, yOf, C('x'), 4, Math.min(3000, Math.ceil(40 * (tau - t0) / T.v) + 20));
    const n1 = Math.floor(tau / T.v + 1e-9);
    if (n1 >= 1 && (n1 - 1) * T.v >= t0 - 1e-9) hbracket(ctx, Xp((n1 - 1) * T.v - tau), Xp(n1 * T.v - tau), yOf(X.v) - 26, C('t'), 'T = ' + fmt(T.v, 2) + ' s');
    ...
    text(ctx, 'time before now (s)', pr, pb + 56, C('t'), { size: 20, weight: 600, align: 'right' });
    ...
    line(ctx, cx + 40, by, pl, by, PAL.ink, 4); dot(ctx, pl, by, C('x'), true, 8);
    if (Math.abs(x) > 0.004) vbracket(ctx, cx - 70, y0, by, C('x'), 'x = ' + sgn(x) + fmt(Math.abs(x), 3) + ' m', -1);
...
(function () {
  ...
  function draw() {
    ...
    block(ctx, cx, by, 80, 56, PAL.ink); text(ctx, 'm', cx, by, PAL.ink, { size: 20, weight: 600, align: 'center' });
    line(ctx, 60, y0, 480, y0, PAL.muted, 2, [10, 10]); text(ctx, 'x = 0', 60, y0 - 18, C('x'), { size: 18, weight: 600 });
    if (Math.abs(a) > 0.03 * amax) { const al = 40 + 120 * Math.abs(a) / amax, s = a > 0 ? -1 : 1; arrow(ctx, cx - 70, by, cx - 70, by + s * al, C('a'), 5); text(ctx, 'a', cx - 70, by + s * (al + 20), C('a'), { weight: 600, align: 'center' }); }
    if (Math.abs(v) > 0.03 * vmax) { const al = 40 + 120 * Math.abs(v) / vmax, s = v > 0 ? -1 : 1; arrow(ctx, cx + 70, by, cx + 70, by + s * al, C('v'), 5); text(ctx, 'v', cx + 70, by + s * (al + 20), C('v'), { weight: 600, align: 'center' }); }
    if (Math.abs(x) > 0.004) { vbracket(ctx, cx + 130, y0, by, C('x')); text(ctx, 'x = ' + sgn(x) + fmt(Math.abs(x), 3) + ' m', cx + 150, (y0 + by) / 2, C('x'), { weight: 600, bg: PAL.panel }); }
    /* the three graphs, the pen at the right */
    const rows = [['x (m)', C('x'), X.v, xOf, 3], ['v (m/s)', C('v'), vmax, vOf, 2], ['a (m/s²)', C('a'), amax, aOf, 1]];
    const l = 620, r = 1300, t0 = Math.max(0, tau - S);
    rows.forEach(([yl, col, top, f, dec], i) => {
      const box = { l, r, t: 100 + i * 225, b: 250 + i * 225 };
      const { X: gx, Y: gy } = axes(ctx, box, [-S, 0], [-top, top], { xl: i === 2 ? 'time before now (s)' : '', xc: C('t'), yl, yc: col, nx: span.n, ny: 2, fx: (val) => (Math.abs(val) < 1e-9 ? 'now' : fmt(-val, S >= 5 ? 0 : 1)), fy: (val) => fmt(val, dec) });
