/* Which section the companion views describe: the focused group's active
   ...
   command aimed at "this view" knows what it means: the view the pointer or the
   keyboard went into, and failing that the view a focused tab is showing. */
import { layoutStore } from '../layout/store.svelte';
import { focusedGroup, focusedSection } from '../layout/model';
import { registry } from './registry.svelte';
import { parseItemKey, pageItem, sectionId, sectionOfItem, type ItemId, type SectionId, type ViewKind } from '../types/ids';
