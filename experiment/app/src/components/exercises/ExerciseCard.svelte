<script lang="ts">
  /* One exercise: the prompt, an answer widget chosen by the answer's type, and
     ...
     concept when clicked and opens a goto card when hovered. */
...
<div class="exercise" class:hot id={domId} {hidden} bind:this={root}>
...
<style>
  ...
  .ex-info{position:absolute;top:8px;right:8px;width:26px;height:24px;display:grid;place-items:center;padding:0;border:1px solid var(--rule);border-radius:5px;background:var(--panel);color:var(--muted);cursor:pointer}
  .ex-info:hover{color:var(--ink);background:var(--soft)}
  .ex-info:focus-visible{outline:2px solid var(--accent);outline-offset:1px}
  ...
  .prompt{margin-bottom:8px;padding-right:34px}
