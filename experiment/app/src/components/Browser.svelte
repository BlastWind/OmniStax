<script lang="ts">
  /* The Open browser: the book as a small file tree in the palette's frame —
     ...
     The box filters the level you stand on; Right and Enter step in, Left and
     an empty Backspace step out, Enter on a document opens it in the group the
     browser was opened for. Opened by the "Open…" command or a tab strip's
     "+"; browser.ts holds the walk, this file only draws it. */
  ...
  import { CHAPTERS, rowsAt, enter, up, crumbs, levelAt, start, keyOfLevel, type Level, type Row } from '../lib/commands/browser';
  ...
  const items = $derived(rank(query, rowsAt(manifest, level), (r) => r.label));
  const trail = $derived(crumbs(manifest, level));
  const iconOf = (r: Row): string => (r.kind !== 'doc' ? ICON.folder : r.doc === 'text' ? ICON.text : ICON.exercises);
  ...
  const goto = (next: Level | null, key: string | null): void => {
    if (!next) return;
    const i = key === null ? -1 : rowsAt(manifest, next).findIndex((r) => r.key === key);
  ...
  const act = (r: Row): void => {
    const next = enter(level, r);
    if (next) { goto(next, null); return; }
    if (r.kind !== 'doc') return;
    openDoc(r.section, r.doc, ui.browser.group ?? undefined); ui.closeBrowser();
  };
  ...
  const onKey = (e: KeyboardEvent) => {
    ...
    if (e.key === 'ArrowRight') { e.preventDefault(); const it = items[sel]; if (it) goto(enter(level, it.item), null); return; }
...
{#if ui.browser.open}
  <div class="browser" role="dialog" aria-label="Open a section" onclick={(e) => e.stopPropagation()} onkeydown={onKey}>
    <div class="crumbs">
