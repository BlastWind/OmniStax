...
with sync_playwright() as p:
    ...
    # 3 open 2.5 via + into group 2
    js('.tabstrip .plus', 1); pg.wait_for_timeout(300)
    print('3 picker:', pg.is_visible('.picker'))
    row=[r for r in pg.query_selector_all('.picker .row:not(.off)') if r.inner_text().startswith('2.5')][0]; row.query_selector('button').click(); pg.wait_for_timeout(2500)
