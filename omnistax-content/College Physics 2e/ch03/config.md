...
| Figures | a sim per idea or result the section introduces; every sketch replaced by a sim, and a sim that lets the reader drag a vector may stand for a whole run of the book's step figures, carrying the first number of the run and the rest as further originals; a photograph kept when the text points at it or it shows the thing the passage is about, dropped when it is a splash image (the New York intersection of 3.1, the Hawaiian map of 3.2 unless the plan finds the text points at it), each listed in the plan (rule 14) |
| Sim sliders | whatever is interesting and variable in the idea: the two legs of a walk, a vector's magnitude and angle, the components of two vectors, a launch speed and angle, a river's and a boat's speed |
| Motion | a projectile flies, a boat crosses a river and a coin falls in the airliner, so those sims run finite loops with the scrubber; a vector diagram that answers its sliders is a still picture with no transport (rule 14) |
| 3D | none; the plane of motion is the canvas, and 3D is reserved for a spatial idea |
| Fixed scales | every scene and graph scale is fixed once from the slider maxima, so a drawing never rescales to keep the same shape: the vector diagrams of 3.2 take the scale that holds the longest walk the sliders allow, and a value past the end of a graph is pinned there and named (rule 24.6, new) |
| Discrete states | the order of addition in 3.2 is a dropdown of the six arrangements, not a slider reading 1 to 6 (rule 26.1, new) |
| Labels | a kind is labelled once, and where labels ride arrows that can lie along one another they go through the drawing layer's labeller, which steps a label out and leaders it when its slot is taken: the names of A, B and R in 3.3. Where three arcs share almost one highest point, as they do within six degrees of 45° in 3.4, only the chosen one is named (rule 26.7, new) |
| Figures that serve exercises | copied over faithfully, no sliders and no motion, and the book numbers none of them, so their eyebrow reads 'Figure' with no number (new): the map of paths that problems 1 and 2 of both 3.2 and 3.3 refer to (built once, in 3.2, and carried on the card of 3.3's problem 1, so that no book figure is kept twice), the five galaxies of 3.5, the vector diagrams a keyed problem refers to (3.2's velocities v_A and v_B, 3.3's triangular plot and pilot's flight, 3.5's hockey player) |
