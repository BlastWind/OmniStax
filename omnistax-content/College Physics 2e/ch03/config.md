...
| Exercise placement | an exercise goes with the section that introduces what it tests; the AP items of 3.2 test a projectile's vertical and horizontal velocity, so the keyed second one (the 60º launch) is held for 3.4 with both sections' `exercise_notes` saying so, and the unkeyed first one is left out and named in 3.2's notes; the 3.1 AP items stay in 3.1 |
| AP test prep | included where keyed; an unkeyed AP item (3.1's second, 3.2's first, 3.4's) is left out and named in the notes |
