...
marked. The chapter pass of 2026-09-15 brought the lines the build settled
differently to what was built: the folds the seven sections chose, the
figures that move, the images the exercise cards carry, the two items whose
keys a card cannot check, the titles the book prints over Tables 20.1 and
20.2, and three settings the build needed and the proposal did not name, the
numbering of the worked examples, the way an ohm is written inside math, and
the anchors.
...
| Prose | verbatim, the book's own slips included (the missing $n$ in Example 20.3's solution, the spelling of Table 20.2's title, the title of Table 20.1, the fourth AP item of 20.3 that asks for the resistivity of the 1 Ω wire where its stem gives 3 Ω and 11 Ω, the unlabelled part (a) of Example 20.7, and the Critical Thinking key of 20.7 that writes $3.25\times10^{-3}$ J/s for a stem of 3.25 MJ; each named in the section's `notes` and in `exploration.md` under Errata); objectives, summary, glossary pulled into the tables and views; the book's boxed notes kept verbatim (the seven boxes `exploration.md` § Notes lists) |
| Tables | three numbered: Table 20.1, whose title the book prints as "Resistivities $\rho$ of Various materials at 20 °C", and Table 20.2, whose title it prints as "Tempature Coefficients of Resistivity $\alpha$", both in 20.3, and Table 20.3 Effects of Electrical Shock as a Function of Current in 20.6, each rebuilt as a `div.book-table` whose eyebrow is the book's number, its italic group rows (Conductors, Semiconductors, Insulators) kept and its footnotes kept as the book prints them; two unnumbered, the voltage-and-current table inside 20.2's second AP item and the three-row wire table inside 20.3's fifth AP item, which travel in those items' prompts as small tables |
...
| Worked examples | the chapter prints ten, numbered as the publisher numbers them, straight through the chapter and not restarted per section: 20.1, 20.2 and 20.3 in 20.1, 20.4 in 20.2, 20.5 and 20.6 in 20.3, 20.7 and 20.8 in 20.4, and 20.9 and 20.10 in 20.5; 20.6 and 20.7 print none. Every reference to an example, in the prose, a caption, a problem or a concept's evidence, carries the same number |
...
| Motion | decided per figure (rule 14). Several ideas of this chapter have a clock in them and the figures that draw them move, and the plan line must say so and why: the charge crossing the marked area of a wire while the counter beside it fills, the loop being the time Δt itself (20.1), the crowd of free electrons drifting through the lattice while each electron rattles (20.1, where the two speeds at once are the idea), the pulse that runs the length of a wire while the electrons barely move (20.1, the same figure), the current going round the simple circuit (20.2), the AC source reversing and its power curve sweeping (20.5, whose whole subject is a periodic time), the ions crossing a membrane until the Coulomb force halts them, the action potential rising and falling, the impulse running down an axon (20.7), and the depolarization wave crossing the heart beside the ECG trace it writes (20.7). Nine figures move in all, two in 20.1, one in 20.2, two in 20.5 and four in 20.7; the other twenty-one are still. The rest are still: the cylinder and its resistance, the resistivity table, the filament's resistance against temperature, the transmission line's losses, the shock bands and the fuse and breaker each answer their sliders, register no cycle and get no transport |
...
| Figures that serve exercises | the book's own rules give two ways of carrying an image an exercise refers to, and this chapter uses the second: the unnumbered images of 20.1, 20.3 and 20.4 travel on the `figure` field of the exercise cards that refer to them, as Chapters 4, 9, 12, 15 and 19 do. An image whose problem is unkeyed and so left out is not copied, which left eight cards carrying seven images: the defibrillator on 20.1's fourth problem and again on the defibrillator problem set in 20.4, the SPEAR ring, the four wires of 20.3's second AP item, the rectangular bar of 20.3's second conceptual question, and the energy graph of 20.4's first AP item, the on-demand water heater and the electric car |
| Colour coding | two new types, `current` (A) and `resistance` (Ω), because the chapter's figures draw them, their sliders carry them and their readouts state them; `voltage` is Chapter 19's, `charge` and `electric-field` Chapter 18's, `power` and `energy` Chapter 7's, `velocity` Chapter 2's, `frequency` Chapter 16's, `temperature` Chapter 13's and `position` Chapter 2's, all used by name and never restaged; the resistivity $\rho$, its temperature coefficient $\alpha$, the free-charge density $n$, the cross-sectional area $A$, the number of electrons and every specific heat and mass the Integrated Concepts problems bring in stay untyped and in ink. Symbol rows added are listed under "Symbols" below, and `ch20/COLOR.md` carries the bindings the seven pages were built with |
| The ohm inside math | `\Omega`, never the literal character, wherever a resistance is written in math: in `text.html`, in a `prompt` or a `solution` of `section.json`, in a `latex` or `ktex` of `chapter.json`, and in every readout a figure writes. The literal Ω is for plain text alone, which is where the book's rules put a caption attribute, a slider's `unit`, a variable's `unit`, a glossary definition and the canvas labels a figure draws |
...
| AP test prep | included; the chapter's 15 AP items sit in 20.1, 20.2, 20.3 and 20.4, and 20.5, 20.6 and 20.7 have none. An unkeyed AP item is kept as an open item with its options as the book prints them and an AI-marked suggested approach, never as a graded choice. 20.4's first AP item reads four unnumbered answer graphs by reference, and since an exercise card carries one image it keeps the stem's circuit-and-energy graph alone and its four answers are written out in words exactly as the book draws them. An item whose key is two of its options (20.3's second AP item, which asks the reader to select two answers) is kept as an open item with the book's key as its solution, since a card checks one choice |
...
| Answers to book problems | book answer key only; never generated; the 50 unkeyed problems are left out and named in the notes, among them 20.4's and 20.6's Construct Your Own Problem items and the eighteen unkeyed problems of 20.4. A keyed answer that is a range rather than a number (20.3's fourth problem, whose key is −5 °C to 45 °C) is kept as an open item with the book's key as its solution, as a "show that" derivation is |
...
| Anchors | every variable row and every equation row of the seven sections carries one, written by the chapter pass from the section plans and naming a span of the section's own `text.html`; the glossary table carries no `anchor` field, so 20.3's two glossary rows take none |
