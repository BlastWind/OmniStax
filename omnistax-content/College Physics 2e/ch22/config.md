# Config: College Physics 2e, Chapter 22
...
left for review after, as Chapters 1 to 21 did it. Each line is a setting and
its value. Lines that repeat the Chapter 1 to 19 configs are unchanged unless
marked.

| Setting | Value |
|---|---|
| Chapter | 22 Magnetism, modules m42365 (introduction), m42366, m42368, m42370, m42372, m42375, m42377, m42398, m42380, m42382, m42386, m42388 |
| Front matter | the chapter introduction (m42365) is a page of its own in `ch22/intro/`, listed before 22.1 (rule 21), built in the prep pass |
| Unit of work | one section = one page; sections never folded (rule 11); 22.1, which has one conceptual question and no equation, and 22.3, which has two figures and no equation, stay pages of their own, and 22.11, which is an applications section with 41 exercises, stays one page |
| Order | 22.1 to 22.11 in book order, built in parallel by one agent per section |
| Loop | plan file → build → validator, all eleven sections in one wave; review after |
| Prose | verbatim; objectives, summary, glossary pulled into the tables and views; the book's eight boxed notes kept verbatim where the book stands them (Universal Characteristics of Magnets and Magnetic Poles, Misconception Alert: Earth's Magnetic Poles and the refrigerator-magnet Take-Home Experiment in 22.1, Electric Currents and Magnetism in 22.2, Making Connections: Concept of a Field in 22.3, Making Connections: Charges and Magnets in 22.4, Making Connections: Relativity in 22.9, The Ampere in 22.10); 22.9's untyped conceptual question and 22.11's Critical Thinking key, which prints the electron mass as $9.11\times10^{-11}$ where it means $9.11\times10^{-31}$ kg, are carried as the book writes them and named in the sections' `notes` |
| Tables | one, unnumbered: the iron-and-lead dipole table inside 22.2's fifth AP item, which travels in that item's prompt as a small table with Iron and Lead as its columns and the four lettered rows described in words, since its cells are eight arrow images the exercise does not need reproduced |
...
| Exercise placement | an exercise goes with the section that introduces what it tests; 22.5's one AP item asks for the force on a *wire*, which 22.7 introduces, and goes to 22.7 with `source_section: "22.5"`, both sections' `exercise_notes` saying so. 22.11's 41 exercises stay where the book prints them, tagged to the concepts they test across 22.5 to 22.10, because the book gathers them there as the chapter's review set, and 22.11's `exercise_notes` records this. `exploration.md` § Exercises that belong to another section lists the cases |
...
| Book manifest | `ch22` after `ch21` in `book.json` chapters (after `ch19` if Chapters 20 and 21 have not merged first; `ost merge` sorts by number), added by `ost merge college-physics-2e 22` |
...
- `B` is Chapter 3's **vector B** with `\kB`, a position, and `B_x` and `B_y`
  its components; `B_bulk` is Chapter 5's **bulk modulus**, also written $B$.
  The magnetic field is `B_mag` (LaTeX `B`, type `magnetic-field`, macro
  `\kBmag`), and 22.10's field of the first wire is `B_1mag` (`B_1`,
  `\kBmagone`).
- `I` is Chapter 10's **moment of inertia** with `\kI`, and `I_1` and `I_2` are
  Chapter 17's **intensities**. The current is Chapter 20's `I_curr` (`\kIcur`)
  and the two currents of 22.10 are Chapter 21's `I_onecurr` (`\kIcurone`) and
  `I_twocurr` (`\kIcurtwo`); all three landed in this wave and are used by
  name, never restaged.
- `ε_0` is Chapter 19's **permittivity** and Chapter 21 writes the emf as
  `emf`, LaTeX `\mathcal{E}`, a voltage with `\kemf`. The Hall emf of 22.6 is
  written `\varepsilon` where the book writes it, so this chapter stages
  `emf_hall` (LaTeX `\varepsilon`, type `voltage`, macro `\kemfhall`) rather
  than changing Chapter 21's row or the book's glyph.
- `w` is Chapter 4's **weight** with `\kwgt`. The width of 22.8's loop is
  `w_loop` (LaTeX `w`, untyped, no macro), a length and so in ink.
- `N` is Chapter 4's **normal force** with `\kN`. The number of turns is
  Chapter 13's untyped `N_count` (LaTeX `N`) and is used as it stands, as is
  the untyped `n` for the turns per unit length.
- `R` is Chapter 6's **circumference-and-radius** row, a position with `\kR`,
  and is the radius of 22.9's loop as it stands; `r_curv` (LaTeX `r`, position,
  `\kr`) is the radius of a charge's circle in 22.5 and the shortest distance
  to a wire in 22.9 and 22.10, as Chapter 6 keyed it.
- `E` is Chapter 7's **total energy** with `\kE`; the electric field is Chapter
  18's `E_field` (`\kEf`) and is used by name in 22.6 and 22.11.
- `l`, `A`, `θ`, `m`, `v`, `q`, `F`, `τ`, `m_e` and `q_e` are used as they
  stand. The magnetic force is Chapter 4's `F`, since a magnetic force is a
  force and takes that hue; the torque on a loop is Chapter 9's `τ`.

Rows this chapter stages, with the sections that write them:

| sym | LaTeX | type | macro | where |
|---|---|---|---|---|
| `B_mag` | `B` | magnetic-field | `\kBmag` | 22.3, 22.4, 22.5, 22.6, 22.7, 22.8, 22.9, 22.10, 22.11 |
| `B_1mag` | `B_1` | magnetic-field | `\kBmagone` | 22.10 |
| `w_loop` | `w` | — | — | 22.8 |
| `μ_0` | `\mu_0` | — | — | 22.9, 22.10 |
| `emf_hall` | `\varepsilon` | voltage | `\kemfhall` | 22.6 |

Five rows and one type, `magnetic-field`. The permeability of free space is
untyped and carries no macro, as Chapter 13's `k_boltz` and Chapter 19's
`ε_0` do.

Rows used as they stand: `F` (`\kF`), `v` (`\kv`), `q` (`\kq`), `q_e`
(`\kqe`), `m_e`, `m`, `A`, `l`, `θ`, `n`, `N_count`, `R` (`\kR`), `r_curv`
