...
Figure pass (2026-09-15, Claude Fable 5.1): in `sim-sun-earth` the value of a negative bar was set below its foot, where at the lowest $\Delta S_\text{syst}$ it lay across the axis name beneath; a negative bar's value now stands just above the zero line, as the Sun's small loss already did. The other four figures were checked at every state and stand as built.
