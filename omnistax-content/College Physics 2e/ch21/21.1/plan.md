...
Figure pass, 2026-09-15 (Claude Fable 5.1): the four schematics are redrawn at the book's symbol conventions. Every resistor is a zigzag with its name above and its value below (or both on one side of a vertical one), the source is one cell with a long thin positive plate and a short thick negative one and its signs beside it, every current arrow lies beside its wire on the inside of the loop and never on it, and every label is off every wire. Figure 21.2 gains junction dots on its parallel branches and a branch arrow on each; Figure 21.3 + 21.4 + 21.6 loses the collision between the third resistor's value and the equivalent source's voltage, gains junction dots and branch arrows in the mixed wiring, and its first branch carries its labels on the right so they clear the cell's signs; Figure 21.5's rows sit either side of the top wire as the book draws them, its labels no longer sit on the wires, and R₇'s name and value are stacked under it clear of the first group's panel; Figure 21.7's lamp is the book's circle with a coiled filament under a soft radial glow in the power hue, its name and value sit left of it and "the bulb" right of it off the wire, and the motor's switch is a hinged blade on two contacts that lifts when the motor is off.
