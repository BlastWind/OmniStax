...
Figure pass, 2026-09-15 (Claude Fable 5.1): every schematic is redrawn at the book's symbol conventions, with zigzag resistors, a one-cell source with a long thin positive plate and its signs, current arrows beside their wires and every label off every wire. Figure 21.27 + 21.28's ammeter on the top wire carries its reading and caption above the wire, clear of the source and the resistors' names, and the voltmeter's captions no longer sit on the bottom wire. Figure 21.29 + 21.30 no longer hangs the movement's own resistance from the galvanometer as a dangling box that connected to nothing; the movement's resistance is written under the meter face, the shunt's caption clears its value, and the canvas is trimmed to what the one line of circuit needs. Figure 21.31 + 21.32's voltmeter caption is set below the loop rather than on its bottom wire.
