...
Figure pass, 2026-09-15 (Claude Fable 5.1). `sim-collision`: the picture now rides with the center of mass of the pair at one fixed scale of 25 units to the meter, so the cars are full size and the road marks slide under them; the mass labels sit on two rows under the marks, the momentum arrows on 320 units at 40,000 kg·m/s, and the two forces are drawn from the point of contact while the bumpers touch. `sim-probe`: the sky is drawn at one true scale (sixty kilometers by seventeen) with kilometer marks, the momentum of the system is anchored at the center of mass rather than floating in a corner, its vertical part is held at the ground line, labels go through the labeller with the headline's rows blocked, and a flight that leaves the picture is pinned at its edge with its reading. `sim-scatter`: the target's velocity arrow is held inside the canvas and both speeds are written beside their arrows. `sim-center-of-mass`: the carts have wheels and their names, and the masses read above the arrows on two rows where the carts are close. `fig-cart-graph` looked at and left.
