...
When the switch is first moved to position 1 (at *$t=0$*), the current is zero and it eventually rises to ${I}_{0}=\text{V/R}$, where $R$ is the total resistance of the circuit. The opposition of the inductor $L$ is greatest at the beginning, because the amount of change is greatest. The opposition it poses is in the form of an induced emf, which decreases to zero as the current approaches its final value. The opposing emf is proportional to the amount of change left. This is the hallmark of an exponential behavior, and it can be shown with calculus that

$$ I={I}_{0}(1-{e}^{-t/τ})\text{(turning on),} $$  {eq:eip-314}

is the current in an *RL* circuit when switched on (Note the similarity to the exponential behavior of the voltage on a charging capacitor). The initial current is zero and approaches ${I}_{0}=\text{V/R}$ with a {term:characteristic time constant} $τ$ for an *RL* circuit, given by

$$ τ=\frac{L}{R}\text{,} $$  {eq:eip-931}

where $τ$ has units of seconds, since $\text{1}\;\text{H}\text{=}\text{1}\;\text{Ω}\text{·}\text{s}$. In the first period of time $τ$, the current rises from zero to $0\text{.}\text{632}{I}_{0}$, since $I={I}_{0}(1-{e}^{-1})={I}_{0}(1-0\text{.}\text{368})=0\text{.}\text{632}{I}_{0}$. The current will go 0.632 of the remainder in the next time $τ$. A well-known property of the exponential is that the final value is never exactly reached, but 0.632 of the remainder to that value is achieved in every characteristic time $τ$. In just a few multiples of the time $τ$, the final value is very nearly achieved, as the graph in [ref:import-auto-id1169737969272](b) illustrates.
...
When the switch in [ref:import-auto-id1169737969272](a) is moved to position 2 and cuts the battery out of the circuit, the current drops because of energy dissipation by the resistor. But this is also not instantaneous, since the inductor opposes the decrease in current by inducing an emf in the same direction as the battery that drove the current. Furthermore, there is a certain amount of energy, $(\text{1/2}){\text{LI}}_{0}^{2}$, stored in the inductor, and it is dissipated at a finite rate. As the current approaches zero, the rate of decrease slows, since the energy dissipation rate is ${I}^{2}R$. Once again the behavior is exponential, and $I$ is found to be

$$ I={I}_{0}{e}^{-t/τ}\text{(turning off).} $$  {eq:eip-578}

(See [ref:import-auto-id1169737969272](c).) In the first period of time $τ=L/R$ after the switch is closed, the current falls to 0.368 of its initial value, since $I={I}_{0}{e}^{-1}=0\text{.}\text{368}{I}_{0}$. In each successive time $τ$, the current falls to 0.368 of the preceding value, and in a few multiples of $τ$, the current becomes very close to zero, as seen in the graph in [ref:import-auto-id1169737969272](c).

:::example {ex:fs-id1169738136126} Calculating Characteristic Time and Current in an *RL* Circuit
(a) What is the characteristic time constant for a 7.50 mH inductor in series with a $\text{3.00 Ω}$ resistor? (b) Find the current 5.00 ms after the switch is moved to position 2 to disconnect the battery, if it is initially 10.0 A.
**Strategy for (a)**
...
$$ τ=\frac{L}{R}=\frac{7.50 mH}{3.00\;Ω}=2.50 ms. $$  {eq:eip-143}
...
**Strategy for (b)**
We can find the current by using $I={I}_{0}{e}^{-t/τ}$, or by considering the decline in steps. Since the time is twice the characteristic time, we consider the process in steps.
**Solution for (b)**
In the first 2.50 ms, the current declines to 0.368 of its initial value, which is

$$ \begin{array}{lll}I & = & 0\text{.}\text{368}{I}_{0}=(0\text{.}\text{368})(\text{10.0 A}) \\ & = & 3\text{.}\text{68 A at}t=2\text{.}\text{50}\text{ms.}\end{array} $$  {eq:eip-316}

After another 2.50 ms, or a total of 5.00 ms, the current declines to 0.368 of the value just found. That is,

$$ \begin{array}{lll}I' & = & 0\text{.}\text{368}I=(0\text{.}\text{368})(\text{3.68 A}) \\ & = & 1\text{.}\text{35}\text{A at}t=5\text{.}\text{00}\text{ms.}\end{array} $$  {eq:eip-822}

**Discussion for (b)**
After another 5.00 ms has passed, the current will be 0.183 A (see [ref:fs-id1169738227350]); so, although it does die out, the current certainly does not go to zero instantaneously.
:::
In summary, when the voltage applied to an inductor is changed, the current also changes, *but the change in current lags the change in voltage in an RL circuit*. In [Reactance, Inductive and Capacitive](module:m42427), we explore how an *RL* circuit behaves when a sinusoidal AC voltage is applied.
...
$$ I={I}_{0}(1-{e}^{-t/τ})\text{(turning on).} $$  {eq:eip-976}
...
- In the first time constant $τ$, the current rises from zero to $0\text{.}\text{632}{I}_{0}$, and 0.632 of the remainder in every subsequent time interval $τ$.
- When the inductor is shorted through a resistor, current decreases as
        

$$ I={I}_{0}{e}^{-t/τ}\text{(turning off).} $$  {eq:eip-654}

Here ${I}_{0}$ is the initial current.
- Current falls to $0\text{.}\text{368}{I}_{0}$ in the first time interval $τ$, and 0.368 of the remainder toward zero in each subsequent time $τ$.

## Problem Exercises {section:problems-exercises}

:::exercise {fs-id1169738227350} type=problems-exercises 
PROBLEM:
...
:::exercise {fs-id1169737723038} type=problems-exercises 
PROBLEM:
...
:::exercise {fs-id1169737788008} type=problems-exercises 
PROBLEM:
...
:::exercise {fs-id1169736581925} type=problems-exercises 
PROBLEM:
Verify that after a time of 10.0 ms, the current for the situation considered in [ref:fs-id1169738136126] will be 0.183 A as stated.
:::

:::exercise {fs-id1169736596063} type=problems-exercises 
PROBLEM:
...
SOLUTION:
0.100 s to $1.00\times {\text{10}}^{\text{–18}}\;\text{s}$
:::

:::exercise {fs-id1169737762374} type=problems-exercises 
PROBLEM:
...
:::exercise {fs-id1169737723042} type=problems-exercises 
PROBLEM:
...
:::exercise {fs-id1169738248980} type=problems-exercises 
PROBLEM:
The 5.00 A current through a 1.50 H inductor is dissipated by a $\text{2.00 Ω}$ resistor in a circuit like that in [ref:import-auto-id1169737969272] with the switch in position 2. (a) What is the initial energy in the inductor? (b) How long will it take the current to decline to 5.00% of its initial value? (c) Calculate the average power dissipated, and compare it with the initial power dissipated by the resistor.
:::

:::exercise {fs-id1169737787416} type=problems-exercises 
PROBLEM:
...
:::exercise {fs-id1169737851564} type=problems-exercises 
PROBLEM:
...
## Glossary
- {def} **characteristic time constant**: denoted by $τ$, of a particular series *RL* circuit is calculated by $τ=\frac{L}{R}$, where $L$  is the inductance and  $R$ is the resistance
