...
| Formulas | `ch06/chapter.json`: seven equations, all in 6.2, with the book's own three Key Equations and the photon relation `important` and the three derivation steps not; fourteen variable rows. The chapter pass of 2026-09-12 wrote every `anchor` from the section plan: the photon relation and the quantized energies land on `6.2-stationary-states`, the general energy expression on `6.2-hydrogen-like`, the transition energy and the Rydberg constant on `6.2-rydberg-recovered`, and the orbit radius and the ionization limit on `6.2-orbit-radius`. The ionization limit also gained the `ktex` its section plan asked for, so that the formula sheet colours its energies as the page does |
