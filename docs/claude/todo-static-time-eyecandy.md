...
3. Prompts that survive a few iterations get moved from the chat into
   `docs/claude/prompts/*.md`. That folder *becomes* the pipeline over time.
...
- [x] Pick the first textbook: OpenStax College Physics 2e (CC BY 4.0), at `experiment/college-physics-2e_-_WEB.pdf` (gitignored).
...
- [x] Experiment outputs live in `experiment/`: `toc.md`, `EXPERIMENT-LOG.md`, and `ch<NN>/` per chapter (`text.txt`, rendered `pages/`, then one folder per pass).
...
- [x] Render the hand-transformed section: markdown, equations, color-coded variables (`experiment/ch02/2.5/index.html`).
...
- [x] Build widgets by hand (7 in 2.5). Archetypes look viable; see EXPERIMENT-LOG. Learn what a declarative widget spec has to contain and whether an "archetype" approach (plotter, vector field, molecule viewer...) will cover most figures.
...
- [ ] Donation and attribution pages.
