...
Omnia's contents are built from open-source textbook pdfs, primarily from LibreTexts. AI must not alter the tone of the original writing. AI enhances textbooks in the following pipeline:
...
Then the Omnia frontend compiles these processings into a standard curriculum experiences.

## Education Literature Omnia takes inspiration from
Cognitive Load Theory:
- Extraneous load: Time spent "searching" due to bad organization of the material, e.g., flipping pages because a diagram and its analysis paragraph is split, is time wasted. How we can reduce this load is my original inspiration for making Omnia Interactive diagrams with inline formulas, color-coded variables, a floating panel to keep all relevant definitions help us cut down on this load.
...
Bloom's Taxonomy: Remember, Understand, Apply, Analyze, Evaluate, Create helps Omnia in classifying the quality of an exercise question. We won't involve too many advanced questions that can involve Evaluate and Create until we involve AI graders.
...
- Pre-learning is an educational life hack. Positioning Omnia as a pre-learn tool may inspire more than it as a replacement for textbooks if Omnia wants to make itself way into the general curriculum.
- Only exercises contribute XP.

## Pipe dream and Omnia's Roadmap
1. Static-time eyecandy. A dozen courses. Great visuals, interactives, and edtech user experience. Solid `pdf->Omnia course` extraction pipeline. Basic mastery point system. The selling point of Omnia is in the UX and interactive simulations.

From this point on, Omnia should run on donations because the servers will be expensive.

2. Omni. If we have Omnia courses covering 90% of the (American) highschool through university STEM curriculum, which I think a hundred courses will do. It is time to start thinking beyond textbook-by-textbook conversion and beyond highschool through university STEM.
